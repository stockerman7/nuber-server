...
결과는 "당신은 운전자가 아닙니다." 라는 결과로 출력된다. 왜 이런 결과가 나온 것일까? 결과적으로 이 `UpdateRideStatus` 는 승객 측 사용자가 탑승 상태를 변경한 것이다. 자세히 보면 `rideId: 2` 는 운전자의 식별자이다. 그런데 JWT Header 는 승객 측의 것이고 탑승 상태 변경을 요청한 것이기 때문에 상태를 변경할 수 없다고 나온다. 흥미롭게도 `rideId` 는 운전자로 등록되어있는 식별자인데 JWT Header 는 승객의 것이다.

<img src="https://drive.google.com/uc?id=1UoHWgiwtOMsWjEIOq3IhbdZUa4LGmcKw" alt="UpdateRideStatus Test 02" width="960">

그래서 승객의 JWT Header 가 아니라 운전자의 JWT Header 로 변경해 다시 상태 변경 요청을 해보자. 그러니 정상적으로 변경되었다고 출력된다. 최종적으로 `GetRide` 로 운전자 측 사용자가 제대로 적용되었는지 확인해 본다.
...
이제 우리는 Ride Status 를 받아보기 위한 Subscription 을 작성해보자. `src/api/Ride` 디렉토리에 다음과 같은 파일을 만든다.
...
`payload` 는 `RideStatusSubscription.graphql` 에서 Subscription Type 으로 정의된 `RideStatusSubscription: Ride` 속성을 알고 있다. 결국 `Ride` 를 가져오면 거기에 `driverId`, `passengerId` 가 담겨져 있을 것이다. 발행측에선 탑승 상태 정보를 전달하고 구독측에선 운전자든 승객이든 식별된 사용자인지 판별해 구독한다.
          ...
          try {
            ...
            if (args.status === "ACCEPTED") {
              ...
              if (ride) {
                ...
                user.isTaken = true;
                user.save();
                // 운전자가 탑승을 수락했기 때문에 탑승한 승객과 새로운 채팅방이 생성된다.
                await Chat.create({
                  driver: user,
...
채팅방에 어떤 탑승자가 있는지 확인해야 한다. 그런데 우리는 Chat 과 Ride 간에 관계를 설정하지 않았다. 관계를 설정해야 어느 채팅방에 속해있는지 알 수 있다. 그리고 채팅방과 사용자는 1 대 1 관계다. 그래서 `@OneToOne` 으로 관계를 만들어야 한다.
...
하나의 `Chat` 은 한명의 `Ride` 를 가질 수 있다. 그리고 `ride.chat` 으로 탑승자의 채팅방을 알 수 있다. 그리고 `rideId` 를 통해 어떤 대상과 채팅을 하고 있는지 식별하는게 좋을 것이다.
...
`Ride` 와 `Chat` 의 관게의 주인은 `Ride` 이다. 그래서 `Ride` 에 `@JoinColumn()` 을 해준 것이다. 그리고 처음부터 `Ride` 에는 `Chat` 이 없으니 `{ nullable: true }` 로 설정한다. 또한 어떤 채팅방에서 대화를 하는지 식별하는 `chatId` 가 있으면 좋을 것이다. 결국 `Ride` 상태가 `"REQUESTING"` 상태라면 `Chat` 이 없는 것이 정상이다. 그리고 GraphQL 에도 추가된 속성들을 각각 넣는다.
...
그런데 실제로 사용자가 탑승 요청을 하려고 한다면 `Ride` 가 해당 `Chat` 을 알도록 해야한다. 필요한 것은 `UpdateRideStatus` 에서 생성된 채팅방을 `Ride` 와 연결시키는 작업이다.
...
`const chat` 이라는 변수에 새롭게 만들어진 채팅방을 저장하고 그 채팅방을 `ride.chat` 으로 `Ride` 와 연결해 준다. 이제 탑승자는 자신이 어느 채팅방에 있는지 안다.

----

## #1.84 Testing GetChat Resolver

<img src="https://drive.google.com/uc?id=197_wIxAtnPBOgZHdkiKb8P5uGaNTKEOZ" alt="GetChat Testing 01" width="960">

<img src="https://drive.google.com/uc?id=1v3h7Y3z9SupqxFhvKP94yTo_MwaW1ocD" alt="GetChat Testing 02" width="960">

위 그림처럼 `RequestRide` 탑승 요청이 이루어지면 `UpdateRideStatus` 로 탑승 수락이 이루어져야 `Chat` 이 생성된다. 그리고 생성된 `Chat` 은 `Ride` 와 연결될 것이라고 예상된다.

<img src="https://drive.google.com/uc?id=1kG8fioCWqblALh1xkXHLUPy6pvO56e46" alt="GetChat Testing 03" width="960">

결과에서 처럼 `GetRide` 로 `chatId` 가 생성되었는지 여부를 확인해보면 아직 없다는 것이 드러난다.

<img src="https://drive.google.com/uc?id=1OwXJ0DKJhhZe3r5sMbW5ZZE-GP60IOgN" alt="GetChat Testing 04" width="960">

<img src="https://drive.google.com/uc?id=178l6FVHZqbyQ2J0dB8Qv-2n9jPYZC67l" alt="GetChat Testing 05" width="960">

`UpdateRideStatus` 에서 탑승 상태가 `"ACCEPTED"` 로 변경되고 `GetRide` 에서 `chatId` 가 생긴 것을 볼 수 있다. 그렇다면 채팅방이 생성되었을 것이다. `GetChat` 으로 확인해보자.

<img src="https://drive.google.com/uc?id=1M1jaXRwahJvBORrIgZ146eOZUgXS_yrv" alt="GetChat Testing 06" width="960">

<img src="https://drive.google.com/uc?id=1ARlR8EvDgCXHqMPhoUxM6x06u_ReEgpd" alt="GetChat Testing 07" width="960">
...
<img src="https://drive.google.com/uc?id=1OYSQQKlOLYIklyA3a8PyzyYmbZzb1taV" alt="GetChat Testing 08" width="960">

만약 `Chat` 의 대화 내용인 `Messages` 가 무엇인지 알고 싶다면 `GetChat` 에서 `relations` 속성으로 `["messages"]` 를 추가해야 한다. `{ relations: ["driver", "passenger", "messages"] }` 처럼 수정하고 다시 조회를 해보면 다음과 같은 결과가 나온다.

<img src="https://drive.google.com/uc?id=1dtjo6uGIoM9PRQYsLPXy9BdejncFQKhe" alt="GetChat Testing 09" width="960">

이처럼 관계 연산으로 대화 메세지 또한 연결된다.

----

## #1.85 SendChatMessage Resolver

채팅방에 메세지를 보내는 Resolver 를 작성해보자. `SendChatMessage` 는 `chatId` 로 어떤 채팅방인지 식별하고 `text` 로 메세지 내용을 보내는 인자를 가진다.
...
테스트를 해보자. `text`, `chatId` 를 인자로 메세지를 보내는 `SendChatMessage` 를 실행한다.

<img src="https://drive.google.com/uc?id=1BFsc691wNU2-mRYLQwgpOxrei_qhF2QG" alt="SendChatMessage Test 01" width="960">

작동한 것이 맞는지 확인은 `GetChat` 에서 `chatId` 로 조회하면 제대로 작동하는 것을 확인할 수 있다.

<img src="https://drive.google.com/uc?id=1mgH4o9isBWPK_fBU6PXAXbP3s7oJAhzU" alt="SendChatMessage Test 02" width="960">

----

## #1.86 MessageSubscription

채팅방에 메세지를 사용자에 따라 보여주기 위한 보다 효율적인 방법은 발행과 구독을 이용하는 것이다. 그러기 전에 우선 `Message.graphql` 에 `chatId` 를 추가해 해당 메세지가 어느 채팅방에 해당하는 것인지 알아야 한다.
...
다음으로 `MessageSubscription` 을 작성하도록 하자.
...
- `"newChatMessage"` 를 구독한 측에선 `payload` 로 들어온 `Message` 인스턴스 객체를 받을 것이다.
- 그러면 그 메세지 객체 안에 `chatId` 를 이용해 해당 `Chat` 을 조회하고 가져와 식별한다.
- `true` 냐 `false` 냐에 따라 필터해 필요한 메세지만 받거나 안받을 것이다.

그렇다면 발행하는 측은 어디서 담당할까? 바로 `SendChatMessage` 가 된다. 메세지를 보내는 측이 바로 발행하는 측이 된다.
...
이제 테스트를 해보자. `MessageSubscription` 을 실행시켜 구독한다. 하단에 `Listening...` 된다.

<img src="https://drive.google.com/uc?id=1YI7eZgwUfCX6YctKFqSRgqFYxVVwIW62" alt="MessageSubscription Test 01" width="960">

`SendChatMessage` 를 이용해 메세지를 전달한다.

<img src="https://drive.google.com/uc?id=1jH6n0mbErWUdVXV-NTgMzsa_LWxr24Bc" alt="MessageSubscription Test 02" width="960">

`MessageSubscription` 으로 다시 돌아오면 보낸 메세지가 온 것을 확인할 수 있다.

<img src="https://drive.google.com/uc?id=1bUdfm79Q-zFRJfDhL2ixiKcrrtHyAH5s" alt="MessageSubscription Test 03" width="960">

다시 한번 더 메세지를 보내보자.

<img src="https://drive.google.com/uc?id=14wnmWw-m_TwA_B8mXmZMgbWCPff4j6IW" alt="MessageSubscription Test 04" width="960">

다음 메세지도 잘 도착했다. 이로써 사용자간에 메세지 전달이 용이하게 되었다.

<img src="https://drive.google.com/uc?id=1gA0L6k8i4vFHIOoR5T0ylNnwOJU31qoL" alt="MessageSubscription Test 05" width="960">